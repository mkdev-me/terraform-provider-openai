...
// GroupRolesDataSource - list data source for organization role assignments of a group
...
func (d *GroupRolesDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Use this data source to retrieve a list of all organization roles assigned to a specific group.",
...
func (d *GroupRolesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if adminKey == "" {
		resp.Diagnostics.AddError(
			...
			"The provider must be configured with an Admin API Key (admin_key) to read group role assignments.",
		)
		return
	}
	...
	reqURL := fmt.Sprintf("%s/v1/organization/groups/%s/roles", baseURL, groupID)
